                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($userProgressUpdates as $progress)
                                <div class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition duration-300">
                                    @if($progress->foto)
                                        <img src="{{ asset('storage/' . $progress->foto) }}" alt="Progress Update" class="w-full h-48 object-cover">
                                    @else
                                    ...
                                    <div class="p-4">
                                        ...
                                        @if($progress->foto)
                                            <button onclick="viewProgressImage('{{ asset('storage/' . $progress->foto) }}', '{{ $progress->id_project }}', '{{ $progress->deskripsi }}')" 
